...
declare variable $c:login := doc("login.xml");
...
declare
%updating
%rest:path("/XSLT/anmeldung")
%rest:GET
function c:anmeldung() as element(screenInfo) {
<screenInfo>
  <name></name>
  <pw></pw>
</screenInfo>
...
declare
%updating
%rest:path("/XSLT/lobby")
%rest:POST("{$body}")
function c:lobby($body) {

  let $a := <member>{($body//name)}<memberID>{(g:newID())}</memberID>{($body//pw)}</member>
  let $b := g:insertMember($a)
  return  c:lobby_start()
};
declare
%updating
%rest:path("/XSLT/lobbyStart")
%rest:GET
function c:lobby_start() {
   $c:lobby
};
