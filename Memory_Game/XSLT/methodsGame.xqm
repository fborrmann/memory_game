...
declare variable $g:membersGame := db:open("XSLT_members")//members;

declare function g:newID() as xs:integer {
  xs:integer(xs:integer(ceiling(Q{java:java.lang.Math}random() * 10000)) - 1)
};

(: LOGIN --------------------------------------------------------------------------------------------------------------:)
declare %updating function g:checkMembers($currentMember) {
  (:to do: check if member in db and only if not in there, create id and insert member:)
  for $member in $g:membersGame//member 
  where $member/name = $currentMember/name
  where $member/pw = $currentMember/pw
  return $member
};

declare %updating function g:insertMember($currentMember) {
  (:to do: check if member in db and only if not in there, create id and insert member:)
  let $member := g:checkMembers($currentMember)  
  return(
  	if (exists($member)) then
  		( session:set('regPlayer', xs:integer(string($member/memberID))) )
  	else( let $setsession:= session:set('regPlayer', xs:integer(string($currentMember/memberID))) 
  		  return (insert node $currentMember as first into $g:membersGame) )
  		  )
};
...
declare function g:newGameXML($gameid as xs:integer, $cards as xs:integer, $player1 as xs:string, $player2 as xs:string, $player3 as xs:string, $player4 as xs:string) as element(game)
{
	...
	<lastpair></lastpair>
	<owner>{(session:get('regPlayer'))}</owner>
...
declare function g:SavedGamesList() {
	<savedGames>
  {
	...
	where $game/owner/text()=session:get('regPlayer')
