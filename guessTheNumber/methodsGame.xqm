...
(: open database guessTheNumber, locate ressource within database and navigate to its top element :)
declare variable $g:instancesGame := db:open("guessTheNumber")/instancesGame;
...
declare
%rest:path("/guessTheNumber/test/createGame/range/{$range}")
%rest:GET
%private
...
declare
%rest:path("/guessTheNumber/test/newGame/range/{$range}")
%rest:GET
...
declare %private function g:insertGameNew($game as element(game)) as empty-sequence() {
  let $insertRequest :=
    <http:request method="POST"
      ...
      href="http://localhost:8984/guessTheNumber/intern/insertGameViaHTTP">
...
declare
%rest:path("/guessTheNumber/intern/insertGameViaHTTP")
%rest:POST("{$body}")
%updating
%private
...
declare
%rest:path("/guessTheNumber/test/processGuess/id/{$id}/guess/{$guess}")
%rest:GET
